...
import { useWindowManagerActions, useWindowManagerState } from './WindowManagerContext';
import { PanelRegistry } from './PanelRegistry';

export const PanelManagerForm: React.FC = () => {
  const state = useWindowManagerState();
  const { openPanel, closePanel, minimizePanel, restorePanel } = useWindowManagerActions();
  ...
  const filtered = allPanelItems.filter(item => 
    ...
    item.componentId.toLowerCase().includes(search.toLowerCase())
  );
